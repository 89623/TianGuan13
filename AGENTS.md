...
- **【③ desc 运行期被改写/还原 → 抽取漏(自定义 text 变量)+ Initialize 反查丢失,在 examine 显示点反查】**examine 的 desc 残留英文有两种 gap:① **自定义 examine-text 变量**(`dry_desc` 血迹变干、`extended_desc` 等)持有玩家可见文本、运行期 `desc = dry_desc` 赋上去——这些变量名不在 SINK_VARS → 文本**根本没进目录**(抽取 gap)。修:把这类变量加进 `SINK_VARS`(它们是 type 变量,抽取器从 `ty.vars` 取初始值)。② **`desc = initial(desc)` / `desc = 某变量`**:`/atom/Initialize` 的 `desc = lang_reverse_text(desc)` 只改**初始值**,运行期任何 `desc = …` 重新赋值(尤其 `initial(desc)` 取回的是**编译期英文**)都会把已译 desc **冲掉** → examine 又显英文。修(通用):**examine 显示点** `. += "<i>[lang_reverse_text(desc)]</i>"`(atom_examine.dm,gated locale≠en、已中文幂等 no-op)——覆盖任何运行期 desc,只要文本在目录。**注意**:显示点反查是**整串精确**,故只救**非插值** desc;插值 desc(`"A [dried?"dried ":""]trail of [X]."`)整串非目录键 → 仍需 AC 兜底或 LANG 改写(另一长尾)。
