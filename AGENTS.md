...
- **反查表**：`lang_reverse_text`（整串精确、全量含单词）；`build_i18n_cache` 合并 locale 目录**所有** `*.json`，含手维护反查文件（en+zh 同 key）`_state_words`/`_fallback`(zh-only AC)/`_cargo_groups`/`_examine_tags`/`_paper_blanks`/`_root`/`_templates`(运行期拼接 `{0}` 模板：DNA vault 报告行、电路板 `(Machine Board)` 后缀名、SM 完整性播报——边界模板引擎按锚命中) 等。`lang_unreverse_text` = zh→en（act 回传容错：`map[x] || map[lang_unreverse_text(x)]`）。
