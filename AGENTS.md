...
- `nova-i18n verbs --locale zh-Hans` —— **编译期**把核心安全 verb 的 `set name = "X"` 原地注入译文（加 `// NOVA EDIT CHANGE - ORIGINAL:` 标记）。verb 名是 BYOND 编译期元数据、**唯一不能 locale 门控**的类别；内容守卫=源码原文须严格 == en（防错位/宏误改）、幂等。需先 extract + 翻译；**不在 `resync.sh` 里**（会改核心、需先有译文），手动跑；产物是**可提交的核心源码**（专属中文服，英文构建需 `git checkout` 还原）。
