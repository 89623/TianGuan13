...
- 玩家/管理员可见字符串全部走 `LANG()`，key 由 `nova-i18n key client "<英文模板>"` 校验后写入 `strings/i18n/{en,zh-Hans}/client.json`。
- 新增 6 条 key（`client.7aea08c3c3903602` / `cc8ba79bc21a9d99` / `9f71aac0a8a6810e` / `2a9ef63f1a5c0307` / `9b753d182153e9a3` / `17c151c01f589ad7`），另复用既有的 `client.15bc27b6fe454546`（Confirmation）与 `client.04c7cd22dc6cb17f`（发送提示）。
- **`"Download Folder (Archive)"` 这个选项串刻意不进目录**：它是 `browse_files()` 里的 switch 比较键（值兼标识符），进目录就会进全局反查表并触发碰撞。同理原有的 `"Download Folder"` 也一直是英文。
- `message_admins()` / `log_admin()` 与 `data/log_archives/README.txt` 是管理日志与内部文件，按惯例保持英文。
