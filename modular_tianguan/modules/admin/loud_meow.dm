ADMIN_VERB(cmd_loud_admin_meow, R_NONE, "loudmeow", "Send a meow to other admins (loudly).", ADMIN_CATEGORY_MAIN)
	...
	for(var/client/admin_client in GLOB.admins)
		if(admin_client?.prefs?.toggles & SOUND_ADMINHELP)
			SEND_SOUND(admin_client, sound('modular_nova/modules/emotes/sound/voice/mggaow.ogg')) //Stop using loud mode if you don't need to.
		window_flash(admin_client, ignorepref = TRUE)

	SSblackbox.record_feedback("tally", "admin_verb", 1, "loudmeow") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
