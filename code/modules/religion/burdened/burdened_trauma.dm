...
/datum/brain_trauma/special/burdened/proc/update_burden(increase, silent = FALSE)
	...
	switch(burden_level)
		if(0)
			if (!silent)
				to_chat(owner, span_warning(LANG("datum.29eacac7", list(GLOB.deity))))
		if(1)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.915a17f5", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.a3e63df8", list(GLOB.deity))))
		if(2)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.300b1f72", list(GLOB.deity))))
			else
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.afe36ae7", null)))
		...
		if(3)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.ecb8575a", list(GLOB.deity))))
				...
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.5e10b7fa", null)))
		if(4)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.c930b66d", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.c40b43bf", null)))
		if(5)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.0d9053bf", null)))
			else
				if (!silent)
					to_chat(owner, span_warning(LANG("datum.afe36ae7", null)))
		...
		if(6)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.8710704c", null)))
		...
		if(7)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.a17b135b", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.c40b43bf", null)))
		if(8)
			if (!silent)
				if(increase)
					to_chat(owner, span_notice(LANG("datum.fdcce58b", list(GLOB.deity))))
				else
					to_chat(owner, span_warning(LANG("datum.afe36ae7", null)))
		if(9)
			if(increase)
				if (!silent)
					to_chat(owner, span_notice(LANG("datum.69dd13a0", list(GLOB.deity))))
