...
/mob/living/simple_animal/bot/examine(mob/user)
	...
	. += span_notice(LANG("mob.5ce1dd0b", list(p_Their(), bot_cover_flags & BOT_COVER_MAINTS_OPEN ? "open" : "closed")))
	. += span_info(LANG("mob.8871308b", list(bot_cover_flags & BOT_COVER_MAINTS_OPEN ? "close" : "open")))
	if(bot_cover_flags & BOT_COVER_MAINTS_OPEN)
		. += span_notice(LANG("mob.47b7bd2c", list(p_Their(), bot_cover_flags & BOT_COVER_LOCKED ? "locked" : "unlocked")))
		var/is_sillycone = HAS_SILICON_ACCESS(user)
		if(!(bot_cover_flags & BOT_COVER_EMAGGED) && (is_sillycone || user.Adjacent(src)))
			. += span_info(LANG("mob.15f02894", list(is_sillycone ? "" : "or use your ID on ", bot_cover_flags & BOT_COVER_LOCKED ? "un" : "", p_their())))
	if(paicard)
		. += span_notice(LANG("mob.9f18ea54", list(p_They())))
		if(!(bot_cover_flags & BOT_COVER_MAINTS_OPEN))
...
/mob/living/simple_animal/bot/proc/bot_control_message(command, user)
	switch(command)
		if("patroloff")
			to_chat(src, LANG("mob.0ff8bb6d", null))
		if("patrolon")
			to_chat(src, LANG("mob.1561176e", null))
		if("summon")
			to_chat(src, LANG("mob.0216966a", list(user, get_area_name(user))))
		if("stop")
			to_chat(src, LANG("mob.c32e2acb", null))

		if("go")
			to_chat(src, LANG("mob.fa7c0824", null))

		if("home")
			to_chat(src, LANG("mob.1dc4070b", null))
		if("ejectpai")
			return
		else
	...
	switch(action)
		...
		if("hack")
			...
			if(!(bot_cover_flags & BOT_COVER_EMAGGED))
				...
				to_chat(user, span_warning(LANG("mob.0f7db643", list(src, hackables))))
			...
			if(!(bot_cover_flags & BOT_COVER_HACKED))
				to_chat(user, span_bolddanger(LANG("mob.a74119fe", list(src, hackables))))
				return
			bot_cover_flags &= ~(BOT_COVER_EMAGGED|BOT_COVER_HACKED)
			to_chat(user, span_notice(LANG("mob.9f32a733", list(src, hackables))))
		...
		if("eject_pai")
			if(!paicard)
				return
			to_chat(user, span_notice(LANG("mob.daa0023e", list(paicard, initial(src.name)))))
			ejectpai(user)
