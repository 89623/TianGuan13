...
ADMIN_VERB(camera_view, R_DEBUG, "摄像头范围显示器", "Shows the range of cameras on the station.", ADMIN_CATEGORY_MAPPING)
	var/on = FALSE
...
ADMIN_VERB(cmd_show_at_list, R_DEBUG, "显示回合开始 AT 列表", "Displays a list of active turfs coordinates at roundstart.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(cmd_show_at_markers, R_DEBUG, "显示回合开始 AT 标记", "Places a marker on all active-at-roundstart turfs.", ADMIN_CATEGORY_MAPPING)
	var/count = 0
...
ADMIN_VERB(enable_mapping_verbs, R_DEBUG, "启用地图动词", "Enable all mapping verbs.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(disable_mapping_verbs, R_DEBUG, "禁用地图编辑动词", "Disable all mapping verbs.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(count_objects_all, R_DEBUG, "计数全部对象", "Counts the number of objects of a certain type in the game world.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(disable_communication, R_DEBUG, "禁用所有通讯动词", "Disables all communication verbs.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(create_mapping_job_icons, R_DEBUG, "生成职业地标图标", "Generates job starting location landmarks.", ADMIN_CATEGORY_MAPPING)
	var/icon/final = icon()
...
ADMIN_VERB(station_food_debug, R_DEBUG, "统计站内食物", "Counts the number of food items on the station.", ADMIN_CATEGORY_MAPPING)
...
ADMIN_VERB(check_for_obstructed_atmospherics, R_DEBUG, "检查受阻的大气部", "Checks for obstructions on atmospherics machines.", ADMIN_CATEGORY_MAPPING)
