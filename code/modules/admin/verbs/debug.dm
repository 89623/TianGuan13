...
ADMIN_VERB(air_status, R_DEBUG, "空气状态在位置", "Gets the air status for your current turf.", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(cmd_admin_grantfullaccess, R_DEBUG, "授予完全访问权限", "Grant full access to a mob.", ADMIN_CATEGORY_DEBUG, mob/M in world)
	if(!SSticker.HasRoundStarted())
		tgui_alert(user, LANG("datum.f838a6c3", null))
		return
	if(ishuman(M))
...
ADMIN_VERB(cmd_assume_direct_control, R_ADMIN, "接管直接控制", "Assume direct control of a mob.", ADMIN_CATEGORY_DEBUG, mob/M)
	if(M.ckey)
...
ADMIN_VERB(display_overlay_log, R_DEBUG, "显示器叠加层 Log", "Display SSoverlays log of everything that's passed through it.", ADMIN_CATEGORY_DEBUG)
	render_stats(SSoverlays.stats, user)

ADMIN_VERB(init_log, R_DEBUG, "显示Initialize()日志", "Displays a list of things that didn't handle Initialize() properly.", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(debug_plane_masters, R_DEBUG, "编辑/调试平面", "Edit and visualize plane masters and their connections (relays).", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(start_line_profiling, R_DEBUG, "开始行分析", "Starts tracking line by line profiling for code lines that support it.", ADMIN_CATEGORY_PROFILE)
	LINE_PROFILE_START
...
ADMIN_VERB(stop_line_profiling, R_DEBUG, "停止线路分析", "Stops tracking line by line profiling for code lines that support it.", ADMIN_CATEGORY_PROFILE)
	LINE_PROFILE_STOP
...
ADMIN_VERB(reestablish_tts_connection, R_DEBUG, "重新建立到 TTS 的连接", "Re-establishes connection to the TTS server if possible", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(queue_tracy, R_DEBUG, "下一局切换 Tracy", "Toggle running the byond-tracy profiler next round", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(count_instances, R_DEBUG, "计数原子/数据体", "Count how many atom or datum instances there are of each type, then output it to a JSON to download.", ADMIN_CATEGORY_DEBUG)
...
ADMIN_VERB(export_save_to_dev_preference, R_DEBUG, "将保存导出为开发偏好", "Exports your savefile to be used by any guests that connect to your localost.", ADMIN_CATEGORY_SERVER)
