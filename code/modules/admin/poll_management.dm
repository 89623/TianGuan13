...
/datum/admins/proc/poll_parse_href(list/href_list, datum/poll_question/poll)
	...
	if(error_state.len)
		if(poll.edit_ready)
			to_chat(usr, span_danger(LANG("datum.c84c6f46", list(error_state.Join("\n")))), confidential = TRUE)
		else
			to_chat(usr, span_danger(LANG("datum.c606d2ad", list(new_poll ? "initialized" : "submitted", error_state.Join("\n")))), confidential = TRUE)
			if(new_poll)
				qdel(poll)
		return
	if(submit_ready)
...
/datum/poll_question/proc/clear_poll_votes()
	...
	qdel(query_clear_poll_votes)
	poll_votes = 0
	to_chat(usr, span_danger(LANG("datum.586ccfff", list(poll_type == POLLTYPE_TEXT ? "responses" : "votes"))), confidential = TRUE)
...
/datum/admins/proc/poll_option_parse_href(list/href_list, datum/poll_question/poll, datum/poll_option/option)
	...
	if(error_state.len)
		if(new_option)
			to_chat(usr, span_danger(LANG("datum.f62b4da8", list(error_state.Join("\n")))), confidential = TRUE)
			qdel(option)
		else
			to_chat(usr, span_danger(LANG("datum.c84c6f46", list(error_state.Join("\n")))), confidential = TRUE)
		return
