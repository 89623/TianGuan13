...
/obj/machinery/posialert/examine(mob/user)
	...
	if(!COOLDOWN_FINISHED(src, robotics_cooldown))
		. += span_notice(LANG("obj.59e2f474", list(COOLDOWN_TIMELEFT(src, robotics_cooldown) * 0.1)))
		. += span_notice(LANG("obj.9bb8a3c7", list(mute_reason)))
	. += span_notice(LANG("obj.33f99fac", null))
