...
/mob/living/silicon/proc/get_silicon_flavortext(mob/user)
	...
	if (client?.prefs.read_preference(/datum/preference/text/character_ad))
		. += span_notice(LANG("mob.9b49b4b0", list(REF(src))))
	...
	if(temporary_flavor_text)
		if(length_char(temporary_flavor_text) <= 40)
			. += span_notice(LANG("mob.68c90e8e", list(temporary_flavor_text)))
		else
			. += span_notice(LANG("mob.be452a50", list(copytext_char(temporary_flavor_text, 1, 37), REF(src))))
