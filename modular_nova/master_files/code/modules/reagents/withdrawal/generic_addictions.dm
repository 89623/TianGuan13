/datum/addiction/nicotine/proc/trigger_random_side_effect(mob/living/carbon/affected_carbon, seconds_per_tick, strength = 1)
	switch(rand(1, 4))
		...
		if(2)
			to_chat(affected_carbon, span_warning(LANG("datum.60d845f3", null)))
			affected_carbon.adjust_stamina_loss(25 * seconds_per_tick)
		if(3)
			if(strength >= 2)
				to_chat(affected_carbon, span_warning(LANG("datum.faca08ef", null)))
				affected_carbon.set_dizzy(strength * 6 SECONDS)
		if(4)
			if(strength >= 2)
				to_chat(affected_carbon, span_warning(LANG("datum.3c3a63b3", null)))
